...
const CLIPPY_SOLUTIONS = {
  ...
  mentor_clear: { subtitle:"Mentor-Run vollständig abschließen.", steps:[ 'Prüfe den Stand: <code>quests</code> (Noah, Emma, Leo müssen erledigt sein).', 'Zeige Prozesse: <code>ps</code>. Suche nach <code>quest_aktiv</code> (PID 67).', 'Beende den Abschluss-Prozess: <code>kill 67</code>.', 'Bestätige mit <code>mentor_clear</code> und <code>quests</code>, dass der Run wirklich clear ist.' ], hint:'Erklärung: Erst der Kill von quest_aktiv beendet den Mentor-Run final.' },
